...
이 세션에서 파일 탐색(Glob, Grep, Read)은 **현재 프로젝트 폴더(`미래사업-가공\`) 내부로만** 한정한다.

- 상위 폴더(`사업계획\`)나 형제 폴더(`2027년도계획\` 등)의 파일을 읽거나 참조하지 않는다.
- 사용자가 명시적으로 외부 경로를 지정한 경우에만 예외를 허용하며, 그 경우에도 읽기 전 사용자에게 확인한다.
- "이 프로젝트 요약" 등의 요청은 현재 폴더 내부만 탐색하여 답한다.
