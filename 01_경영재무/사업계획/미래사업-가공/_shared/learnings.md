...
## [2026-06-10] [gpu-machining-opportunity]
**교훈**: claude-main(Task sub-agent) 반환 채널이 긴 산출물에서 최종 메시지를 "완료."로 붕괴시켜 본문이 유실될 수 있다. **fresh 호출은 도구를 실행하지만 SendMessage resume은 tool_uses=0으로 도구를 실행하지 않는다** → 긴 산출물은 **fresh 호출에 "Write로 `workers/<role>/result.md`에 저장 후 'saved'만 응답"을 지시**해 반환채널을 우회하라. 평소 "claude-main 파일쓰기 금지" 규칙의 정당한 예외(자기 result.md 한정·채널 우회 목적, log에 기록).
**근거**: 동일 brief 3회 호출(1차 fresh 17 tool_uses, 2·3차 resume)이 모두 "완료."만 반환·output파일도 1줄. 4차 fresh+Write지시(24 tool_uses)에서 result.md 정상 생성. 콘텐츠 생성은 정상, 전달 채널만 문제였음.
**worker**: claude-main(호출 경로), orchestrator(우회 설계)

## [2026-06-10] [gpu-machining-opportunity]
**교훈**: worker 의존 전 **사전점검(pre-flight)**을 하라 — gemini(agy)는 헤드리스 셸에서 인증/세션이 없으면 `--print`가 EXIT0·빈 출력으로 조용히 실패한다(`agy models`도 빈 결과, config 디렉터리에 `bin/`만 존재). GEMINI_API_KEY 미설정 시 폴백도 불가. 실패 시 "독립 조사" 역할은 orchestrator WebSearch로 대체하고 사용자에게 미실행을 **부록이 아니라 본문에서** 고지하라(사용자가 명시 요청한 토폴로지이므로). 알려진 인프라 버그(시스템파일이라 §2 별도 프로토콜, 본 작업 중 미수정): ①`backends.json`의 gemini `-p`가 설치된 agy.exe에서 미정의(`--print`만 수용) ②`call_worker.sh:104` `${!reqenv:-}`가 "invalid variable name" 유발.
**근거**: agy 직접 테스트로 빈 출력·빈 models 확인, config에 bin만. advisor가 "요구 충실도(requirement fidelity)"를 최우선 수정사항으로 지적 — 사용자 요청 토폴로지의 절반(agy)이 미실행됐으면 본문 고지가 필수.
**worker**: gemini(환경 실패), orchestrator(사전점검·대체·고지)

## [2026-06-11] [gpu-machining-opportunity]
**교훈**: 적대적 리뷰는 **양방향 보정**에 쓰라 — 1차엔 워커(claude)가 진입장벽을 *과대*평가했고, 2차엔 유리한 새 입력(강한 회사 프로필)이 들어오자 orchestrator가 *과소*평가(과잉낙관)했는데 codex가 양쪽을 모두 잡아 정직한 중간으로 수렴시켰다. 전형적 과잉낙관 비약 3종 — ①"고객 보유 인증/실적이 신규 도메인의 고객별 승인(AVL/qualification)을 *대체*한다", ②"매출 규모→설비 투자 가능", ③"동일 법인 타 사업부 거래→신규 사업부 영업 레버리지" — 은 모두 **우대요인 vs 별도관문**을 분리하고 단정 대신 조건부로 낮춰야 한다. 유리한 새 입력이 들어올수록 리뷰어를 다시 태우는 것이 가치(새 입력은 새 편향을 만든다).
**근거**: codex v2가 CRITICAL 3건(IATF=AVL대체 둔갑, 기존고객=강력레퍼런스 과장, 로드맵 단축 근거부족)으로 정확히 지적, orchestrator 재검증 결과 11건 전부 진성. never-trust-upstream을 자기 산출물의 낙관 편향에도 적용.
**worker**: orchestrator(재보정), codex-critic(양방향 교정)
