...
Phase 2 완료 후 종합 코드 리뷰(판정: 승인, Critical/High 없음). 아래 반영 완료(커밋 d9e6233).

- [x] **M1** 활동 컬럼을 이름+타입(datetime 계열)으로 검증 선택 + MAX 결과 타입 가드 → `datetime2` 삽입 실패로 인한 활동 적재 중단 방지.
- [x] **M2** DMV 수집 실패를 전면 graceful(None) 처리 → 한국어 로케일·권한/DMV 미가용 등 어떤 경우에도 파이프라인 중단 없음.
- [x] **M3** 프로파일링을 `profile_tables`로 단일 연결 재사용 → 테이블당 connect/close 제거(운영 부하 절감).
- [x] **L3** 식별자 대괄호 인용 시 `]`→`]]` 이스케이프(`db.quote_ident`).
- [x] **L4** `profile_top_n: 0`을 유효값으로 허용(0=프로파일 생략 의도 보존).

### Phase 2 잔여 백로그 (리뷰 Low, 다음 기회에)

- **L1** 프로파일 표본 비결정성 — TOP N에 안정 정렬키(PK/클러스터드) 부여 또는 표본 1회 물질화(임시테이블/CTE)로 분모·분자 일치.
- **L2** `TOP N` 대신 `TABLESAMPLE`(설계 권고) — 대용량 편향 표본 개선. 소량은 전수.
